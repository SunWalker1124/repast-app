package com.rz.repast.redis;

import com.rz.repast.utils.JSONUtil;
import com.rz.repast.staticstatus.StaticCode;
...
public class RedisService<T> {
    ...
    public String set(String key, T value, String nxxx, String expx, Integer time) {
        ...
        if((null != time && 0 < time) && (StaticCode.NX.equals(nxxx) || StaticCode.XX.equals(nxxx)) &&
                (StaticCode.EX.equals(expx) || StaticCode.PX.equals(expx))) {
            // 说明需要去设置失效时间
            return jedis.set(key, JSONUtil.toJsonString(value), nxxx, expx, time);
        } else {
            // 说明不需要设置失效时间
            // 再次判断到底是覆盖，还是不覆盖
            if(StaticCode.NX.equals(nxxx)) {
                // 说明redis中有这个key则不保存
                return String.valueOf(jedis.setnx(key, JSONUtil.toJsonString(value)));
            } else if(StaticCode.XX.equals(nxxx)) {
                // 说明可以直接覆盖
                return jedis.set(key, JSONUtil.toJsonString(value));
            }
        }
        return StaticCode.NO;
    }
    ...
    public Long expire(String key, Integer time, String expx) {
        if(StaticCode.EX.equals(expx)) {
            // 所使用的单位是秒
            return jedis.expire(key, time);
        } else if(StaticCode.PX.equals(expx)) {
